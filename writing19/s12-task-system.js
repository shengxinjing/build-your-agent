...
// s12：任务系统 —— 大目标拆成小任务，排好序，持久化到磁盘。
// s12: a task system — break a big goal into small tasks, order them, persist to disk.
// 区别于 s05 的 todo_write（进程内、当前任务的执行清单）：任务是 .tasks/{id}.json 文件，
// 之间有 blockedBy 依赖图，跨会话可恢复、可被认领（owner）。两者并存，各管一摊。
