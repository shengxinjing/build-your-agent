...
// s14：定时调度 —— 给 Agent 装个闹钟，按 cron 表达式到点自动触发任务。
// s14: a cron scheduler — give the agent an alarm clock; jobs fire on a 5-field cron schedule.
// 调度与执行解耦：cronTick 把到点的 job 推进队列（独立于 agent loop，main 用 setInterval 每秒驱动），
// agent loop 在每轮开头消费队列、注入 [Scheduled] 消息。durable 任务写 .scheduled_tasks.json 跨重启。
// [turn] / [assistant] / [tool result] 日志是教学 trace，方便读者看懂每一轮发生了什么。
