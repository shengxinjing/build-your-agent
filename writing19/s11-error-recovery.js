...
// s11：错误恢复 —— 错误不是终点，是重试的起点。
// s11: error recovery — an error isn't the end, it's where the retry begins.
// 在 s10 基础上，把主循环的 LLM 调用包进三条恢复路径：① 输出截断 → 升 token 再续写；
// ② 上下文超限 → 应急压缩重试；③ 瞬态故障（429/529）→ 指数退避，连续过载切备用模型。
